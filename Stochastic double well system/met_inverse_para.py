    ...
    for i in range(100000+1):
        ...
        if i % 1000 == 0:
            ...
            (loss_result, loss_pde_result, loss_ob_result) = sess.run([loss,
                    loss_f, loss_ob])
            (para0_train,para1_train,para2_train,para3_train,para4_train)=sess.run([para0, para1, para2, para3, para4])
            if temp_loss<min_loss:
                ...
                file_name = (savedir + '/metinver_driftpara_best_model5.pkl')
...
#             loss_record.append(temp_loss)
            ...
            # print ('  %d   total loss: %8.2e pde loss: %8.2e u loss: %8.2e drift loss: %8.2e para1: %8.2e para2: %8.2e' % (i,  temp_loss,loss_pde_result,loss_ob_result,error_drift,para1_train,para2_train) )
#             print ('  %d    %8.2e  %8.2e' % (i,  temp_loss, error_u) )

    # loss_txt_file_name = (savedir + '/loss_record520.txt')
    # np.savetxt(loss_txt_file_name, loss_record, fmt='%.8e', delimiter=',')
...
# plt.title('MET_inverse_driftpara')
# plt.show()
# plt.savefig('MET inverse driftpara11.png')